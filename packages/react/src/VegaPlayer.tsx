...
import { forwardRef, type CSSProperties, useEffect, useImperativeHandle, useRef, useState } from "react";
...
interface VegaPlayerSharedProps {
  ...
  readonly playerOptions?: Omit<VegaPlayerOptions, "mount" | "shell" | "story" | "theme">;
  ...
  readonly onReady?: (handle: VegaPlayerHandle, controller: VegaPlayerController) => void;
...
const appearanceColorScheme = (appearance: VegaPlayerAppearance): CSSProperties["colorScheme"] =>
...
const disposeSession = (session: ActiveSession): Promise<void> => {
  ...
  session.ready.reject(abortError("Vega player was disposed before becoming ready"));
  session.disposal = (async () => {
    ...
    const errors = results
      ...
      .map(({ reason }) => reason);
    if (errors.length) throw new AggregateError(errors, "Failed to dispose Vega React resources");
  })();
...
const startSession = async (session: ActiveSession, reportError: (error: unknown) => void): Promise<void> => {
  prepareStoryAudio();
  const handle = session.handle ?? (await session.ready.promise);
  if (session.disposed) throw new ReferenceError("The Vega React player is disposed");
  if (!session.started) {
  ...
  if (!handle.player.state.playing) trackPlayback(session, handle.player.play(), reportError);
};

export const VegaPlayer = forwardRef<VegaPlayerController, VegaPlayerProps>(function VegaPlayer(
  {
    story,
    project,
    sceneId,
    ...
    (): VegaPlayerController => ({
      ...
      ready() {
        ...
        return session
          ...
          : Promise.reject(new ReferenceError("The Vega React player is not mounted"));
      },
      start() {
        const session = sessionRef.current;
        return session
      ...
      async resume() {
        const session = sessionRef.current;
        if (!session) throw new ReferenceError("The Vega React player is not mounted");
  ...
  useEffect(() => {
    ...
    const engine = providedEngine ?? createVega(engineOptions);
    ...
    void engine
      ...
      .then(async (handle) => {
        ...
        frame = requestAnimationFrame(() => updatePresentation(handle.player.state));
        try {
          callbacksRef.current.onReady?.(handle, controllerForSession(session, reportError));
        } catch (error) {
          reportError(error);
        }
      })
    ...
    return () => {
      ...
      void disposeSession(session).catch((error: unknown) => callbacksRef.current.onError?.(error));
    };
  }, [engineOptions, playerOptions, project, providedEngine, sceneId, shell, story, theme]);

  useEffect(() => {
    const handle = sessionRef.current?.handle;
    if (handle && handle.player.state.autoPlay !== autoPlay) handle.player.toggleAuto();
  }, [autoPlay, status]);
...
const controllerForSession = (session: ActiveSession, reportError: (error: unknown) => void): VegaPlayerController => ({
  get handle() {
