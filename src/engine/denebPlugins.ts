...
export interface DenebRuntimePluginModule {
  ...
  readonly createVegaPlugin?: (
    context: DenebRuntimePluginFactoryContext,
  ) => VegaPlugin | Promise<VegaPlugin>;
}
...
export const loadDenebRuntimePlugins = async (
  ...
  const externalRuntimes = options.externalRuntimeIndexUrl
    ? await loadExternalRuntimeAccess(
        new URL(String(options.externalRuntimeIndexUrl), documentBase()),
        fetcher,
        signal,
      )
    : undefined;
  ...
  const index = parseRuntimePluginIndex(
    await fetchJson(indexUrl, fetcher, signal),
  );
  ...
  for (const entry of index.plugins) {
    ...
    if (
      ...
      plugin.manifest.version !== entry.version
    ) {
      throw new TypeError(
...
const importVerifiedModule = async (
  bytes: Uint8Array,
  options: DenebRuntimePluginLoadOptions,
): Promise<unknown> => {
  const createObjectUrl =
    options.createObjectUrl ?? globalThis.URL?.createObjectURL?.bind(URL);
  const revokeObjectUrl =
    options.revokeObjectUrl ?? globalThis.URL?.revokeObjectURL?.bind(URL);
  if (!createObjectUrl || !revokeObjectUrl) {
    throw new ReferenceError(
      "Deneb runtime plugin loading requires Blob URL support",
    );
  }
...
const materializePlugin = async (
  ...
  if (
    ...
    throw new TypeError(
      `Deneb runtime plugin ${context.id} did not export a Vega plugin`,
    );
  }
  return plugin;
};
...
const loadExternalRuntimeAccess = async (
  ...
  const index = parseExternalRuntimeIndex(
    await fetchJson(indexUrl, fetcher, signal),
  );
    ...
    index.externalRuntimes.map((entry) => {
      const root =
        entry.status === "application-bundle"
          ? safeRelativeUrl(
              requireString(entry.artifactRoot, "artifactRoot"),
              indexUrl,
              true,
            )
          : undefined;
    ...
    fetch: async (
      ...
      if (!file) {
        throw new RangeError(
          `External runtime ${runtimeId} does not declare ${path}`,
        );
      }
      ...
      try {
        ...
        if (bytes.byteLength !== file.bytes) {
          throw new TypeError(
            `External runtime ${runtimeId}/${path} size changed after packaging`,
          );
        }
...
const parseRuntimePluginIndex = (value: unknown): RuntimePluginIndex => {
  ...
  if (
    root.format !== "vega-runtime-plugin-bundle" ||
  ...
  const plugins = root.plugins.map((candidate, index) => {
    ...
    return Object.freeze({
      ...
      configuration: requireJson(
        entry.configuration ?? null,
        `plugins[${index}].configuration`,
      ),
      artifactKind,
      artifact: requirePortablePath(
        entry.artifact,
...
const parseExternalRuntimeIndex = (value: unknown): ExternalRuntimeIndex => {
  ...
  const publiclyRedistributable = requireBoolean(
    root.publiclyRedistributable,
    "publiclyRedistributable",
  );
  ...
  const entries = root.externalRuntimes.map((candidate, index) => {
    ...
    rejectUnknownKeys(
      entry,
      EXTERNAL_RUNTIME_KEYS,
      `externalRuntimes[${index}]`,
    );
    if (
      entry.status !== "application-bundle" &&
      entry.status !== "host"
    ...
    const files = requireArray(
      entry.files ?? [],
      `externalRuntimes[${index}].files`,
    ).map((candidate, fileIndex) => {
      const file = requireRecord(
        candidate,
        `externalRuntimes[${index}].files[${fileIndex}]`,
      );
      rejectUnknownKeys(
        file,
        EXTERNAL_FILE_KEYS,
      ...
      if (!Number.isSafeInteger(bytes) || Number(bytes) < 0) {
        throw new TypeError(
          `externalRuntimes[${index}].files[${fileIndex}].bytes is invalid`,
        );
      }
      const filePath = requirePortablePath(
          file.path,
          `externalRuntimes[${index}].files[${fileIndex}].path`,
        );
      if (filePath <= previousFile) {
        throw new TypeError(
          `externalRuntimes[${index}].files must be unique and sorted`,
        );
      }
      ...
      return Object.freeze({
        ...
        integrity: requireIntegrity(
          file.integrity,
          `externalRuntimes[${index}].files[${fileIndex}].integrity`,
        ),
        bytes: Number(bytes),
      });
    });
    if (
      ...
      throw new TypeError(
        `externalRuntimes[${index}] has no application-private artifact`,
      );
    }
    if (entry.status === "application-bundle") {
      ...
      rejectUnknownKeys(
        authorization,
        EXTERNAL_AUTHORIZATION_KEYS,
        `externalRuntimes[${index}].authorization`,
      );
      if (
        ...
        authorization.acknowledgement !==
          "accepted-for-application-private-build"
      ) {
        throw new TypeError(
          `externalRuntimes[${index}] authorization does not match this application and license`,
        );
      }
      requireIntegrity(
        authorization.receiptIntegrity,
        `externalRuntimes[${index}].authorization.receiptIntegrity`,
      );
    }
    if (
      entry.status === "host" &&
      ...
      throw new TypeError(
        `externalRuntimes[${index}] host requirement contains bundled bytes`,
      );
    }
    ...
    return Object.freeze({
      ...
      version: requireString(
        entry.version,
        `externalRuntimes[${index}].version`,
      ),
      license: requireString(
        entry.license,
        `externalRuntimes[${index}].license`,
      ),
      ...(entry.homepage === undefined
        ? {}
        : {
            homepage: requireHttpsUrl(
              entry.homepage,
              `externalRuntimes[${index}].homepage`,
            ),
          }),
      requiredBy: requireStringArray(
        entry.requiredBy,
        `externalRuntimes[${index}].requiredBy`,
      ),
      ...
      ...(entry.artifactRoot === undefined
        ? {}
        : {
            artifactRoot: requirePortablePath(
              entry.artifactRoot,
              `externalRuntimes[${index}].artifactRoot`,
              true,
            ),
          }),
  ...
  if (
    publiclyRedistributable ===
    entries.some(({ status }) => status === "application-bundle")
  ) {
    throw new TypeError(
      "External runtime redistribution flag contradicts bundled private files",
    );
  }
  return Object.freeze({
...
const validateDependencyOrder = (
  plugins: readonly RuntimePluginIndexEntry[],
): void => {
...
const fetchJson = async (
  ...
  try {
    ...
    throw new TypeError(
      `Deneb index ${url} is not valid JSON: ${
...
const verifyIntegrity = async (
  ...
  if (!globalThis.crypto?.subtle) {
    throw new ReferenceError(
      "Deneb runtime plugin verification requires Web Crypto",
    );
  }
...
const requireStringArray = (
  ...
  const array = requireArray(value ?? [], path).map((entry, index) =>
...
const cloneJson = <T extends VegaJsonValue>(value: T): T =>
